...
**A metadata receiver costs one scan per compilation.** Correctness required it — the previous single-compilation shortcut silently dropped the solution's own extensions on BCL types — but the cost is now N× the measured 59 ms warm / 815 ms cold, unbounded and uncached. On a 50-project solution `get_extension_methods("string")` is seconds warm and considerably worse cold. The obvious optimisation is to scan each referenced *assembly* once rather than once per referencing compilation, since the framework closure is shared and produces identical results every time; only each compilation's own source types genuinely differ. Deliberately not done here: it is a real chunk of work, and shipping the correct-but-slower version beats keeping the fast-but-wrong one. Recorded as a follow-up.
