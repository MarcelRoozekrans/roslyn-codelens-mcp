...
| Catch model | The search yields every binding clause outward from the innermost `try`; the first **unfiltered** one catches (`escapes: false, hasFilter: false`). Filtered (`when`) clauses never stop the search, since they may decline — if only filtered clauses could have caught it, the exception is reported as escaping with `hasFilter: true`. `escapes: false` with `hasFilter: true` is therefore impossible. Bare `catch` and `catch (Exception)` catch everything; `finally` never catches. *(Refined during review: the original wording let a filtered clause shadow a later unfiltered sibling.)* |
...
**Shallowest-call-site-wins:** the walk's visited-set records the depth at which each callee was reached and re-explores it only on a strictly shallower arrival. That keeps the depth budget from depending on call order (a callee first met at the cutoff no longer blocks a later arrival that still has budget), but a callee reached at the same or a greater depth is still explored via the first path only, so an exception's caught/escaping verdict reflects that path. A method called both inside and outside a `try` at the same depth may therefore be reported either way; re-run the flow on the specific caller to disambiguate.

**Async is modelled as synchronous:** a `throw` inside an `async` method does not leave that method at the call site — it is captured in the returned `Task` and re-raised at the `await`. The walk has no await-awareness: it reports such a throw as propagating at the call site, and therefore as caught by any `try` lexically wrapping that call. For an awaited call (`await M()` inside a `try`) that verdict is correct, because the awaited exception really does surface inside the try. For a fire-and-forget call (`_ = M();`, `M().ContinueWith(...)`, an unawaited call in a `finally`) it is wrong: the exception surfaces later, on a different stack, and no enclosing `try` sees it. Detecting the difference needs await/dataflow plumbing this feature deliberately does not carry.
