...
- **`GetProjectHealthToolTests.Counts_ComplexityMatchesUnderlyingTool` is vacuous on the current fixture.** It asserts the composite's `ComplexityHotspots` equals the underlying tool's count at `threshold: 10` — and the test solution's most complex member is `Greeter.ClassifyName` at cyclomatic 5, so both sides are **0** in every project. Measured while following the cognitive-complexity change downstream: the whole complexity dimension of `get_project_health` is currently untested against a non-zero value, which is why that change moved no downstream number. Fixing it means adding a fixture member above 10, which changes absolute counts in the unused/uncovered/public-API suites, so it was left alone rather than bundled into an unrelated change.
